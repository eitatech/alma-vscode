...
Compare `.reversa/version` com a saída de `npm view reversa version` (isso respeita o registry configurado no npm — Artifactory, proxy corporativo ou npm público — em vez de uma URL fixa). Não use `curl`/fetch direto em `registry.npmjs.org`, pois ignora o `.npmrc`. Se a checagem falhar ou estourar timeout, pule silenciosamente. Se houver versão mais nova, informe discretamente após a saudação:
